...
> **Naming.** The release is **LGAE v5.3.0**, the repo is [`dawsonblock/LGAE`](https://github.com/dawsonblock/LGAE). The Python distribution/module is `lgae-v3` / `lgae_v3` — a historical name kept for import stability (renaming would touch every import in 53 source + 32 test files for no functional gain). `pip install lgae-v3` gives you `import lgae_v3` at version `5.3.0`. The CLI is `lgae-v3`.
